...
func _ready():
	...
	for x in get_children():
		...
		if Info.stat["inv"][int(x.name)][0] != 0:
...
func _switch(bol, xname):
	if bol == true:
		...
		if Info.stat["inv"][selected[0]][0] == 0:
			for x in get_children():
				x.pressed = false
		...
		if selected.size() >= 2:
			var first_selected = Info.stat["inv"][selected[0]]
			var second_selected = Info.stat["inv"][selected[1]]
		
			if second_selected[0] == 0:
				Info.stat["inv"][selected[1]] = first_selected
				Info.stat["inv"][selected[0]] = [0, 0]
				...
				Info.stat["inv"][selected[1]][1] += first_selected[1]
				Info.stat["inv"][selected[0]] = [0, 0]
				tw.interpolate_property(get_child(selected[0]).get_child(0), "global_position", null, get_child(selected[1]).rect_global_position, 0.05, Tween.TRANS_EXPO, Tween.EASE_OUT)
				tw.start()
				...
				for x in range(Info.stat["inv"].size()):
					if x == selected[0]:
						Info.stat["inv"][x] = second_selected
					if x == selected[1]:
						Info.stat["inv"][x] = first_selected
...
func _refresh():
	for x in get_children():
		var itin = Info.stat["inv"][int(x.name)]
